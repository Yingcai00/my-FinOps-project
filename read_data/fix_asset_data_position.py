...
subdivide_idx = html_content.find(subdivide_marker, asset_data_start)
if subdivide_idx == -1:
    print("未找到subdivideData开始标记")
    exit(1)

# 从subdivide_idx向前查找];
for i in range(subdivide_idx, asset_data_start, -1):
...
new_html = html_content[:asset_list_vars] + 
           asset_data + '\n\n' + 
           html_content[asset_list_vars:asset_data_start] + 
           html_content[asset_data_end:]
