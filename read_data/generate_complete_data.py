...
# -*- coding: utf-8 -*-
"""
生成完整的JavaScript数据，包括自动生成的成本编码和所有价格数据
"""
...
with zipfile.ZipFile(excel_file, 'r') as z:
    ...
    # 解析单元格 - 同时处理数字类型和字符串类型
    # 匹配所有单元格，包括有类型和没有类型的
    cell_pattern = r'<c r="([A-Z]+)(\d+)"[^>]*?(?: t="([^"]*)")?[^>]*>(?:<v>(.*?)</v>)?</c>'
    cells = re.findall(cell_pattern, sheet_content)
    
    # 组织数据
    data = {}
    for col, row, cell_type, value in cells:
        row_num = int(row)
    ...
    # 保存到文件
    output_file = 'js_complete_data.txt'
