...
def fix_document():
    ...
    # ========== 修正后的当前月-编辑页面需求 ==========
    add_heading(doc, '10. 当前月-编辑页面需求（修正版）', level=1)
    ...
    rows = [
        ...
        ['地区', '单选下拉框', '否', '手动维护时可编辑', '选项：北京、上海、西安、总部（无*号，非必填）'],
    ...
    add_paragraph(doc, '• 纯展示表格：无添加、编辑、删除操作按钮')
    add_paragraph(doc, '• 数据来源：从系统读取，不可手动修改')
    ...
    # ========== 修正后的历史月-详情页面需求 ==========
    add_heading(doc, '11. 历史月-详情页面需求（修正版）', level=1)
    ...
    headers = ['字段名称', '是否必填', '展示方式', '样式']
    rows = [
        ['成本分类', '是', '下拉框（禁用）', '灰色背景#f5f5f5'],
        ['成本池', '是', '下拉框（禁用）', '灰色背景#f5f5f5'],
        ['成本项', '是', '文本输入框（禁用）', '灰色背景#f5f5f5'],
        ['地区', '否', '下拉框（禁用）', '灰色背景#f5f5f5（无*号）'],
        ['公摊池', '是', '下拉框（禁用）', '灰色背景#f5f5f5'],
        ['公摊类型', '是', '下拉框（禁用）', '灰色背景#f5f5f5'],
        ['单价（元）', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['折旧月限', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['折旧月价', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['数量', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['单位', '是', '下拉框（禁用）', '灰色背景#f5f5f5'],
        ['账期', '是', '文本输入框（禁用）', '灰色背景#f5f5f5，cursor: not-allowed'],
        ['月成本（元）', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['公摊比例', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['公摊后月总成本', '是', '数字输入框（禁用）', '灰色背景#f5f5f5'],
        ['描述', '否', '多行文本框（禁用）', '灰色背景#f5f5f5'],
    ]
    add_table(doc, headers, rows)
    ...
    add_paragraph(doc, '• 纯展示：无添加、编辑、删除操作')
    ...
    # 保存文档
    output_path = '成本管理-成本总览页面需求文档_v2.1.docx'
    doc.save(output_path)
    print(f"Word文档修正完成！")
    print(f"保存路径: {output_path}")
    print("\n主要修正内容：")
    print("1. 地区字段明确标注为'非必填'（无*号）")
    print("2. 资源详情Tab页明确为纯展示，无操作功能")
    print("3. 计费资源Tab页功能描述与页面保持一致")
    print("4. 所有表格字段与页面实际结构一致")
